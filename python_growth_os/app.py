...
async def run_once(trigger="manual",body=None):
 ...
 try:return await ex.run({"manual":"Manual Start","scheduled":"Every 15 Minutes"}[trigger],{"json":{"trigger":trigger,"body":body or {}}})
...
async def bundle(x_worker_key:str|None=Header(default=None)):
 auth(x_worker_key); f=ROOT/"data"/"growth_os_v2_local_worker.zip"
 if not f.exists(): raise HTTPException(404,"Worker bundle is not present")
 return Response(f.read_bytes(),media_type="application/zip",headers={"Content-Disposition":"attachment; filename=\"growth_os_v2_local_worker.zip\""})
async def scheduler():
 while True:
  try:
