...
class HTTPClient:
 async def request(self,node,resolved):
    ...
    headers={h["name"]:str(h.get("value","")) for h in p.get("headerParameters",{}).get("parameters",[]) if h.get("name")}
    cred=next(iter((node.get("credentials") or {}).values()),{}).get("name"); headers.update(credential_headers(cred)); params=credential_query(cred)
    ...
    fmt=p.get("options",{}).get("response",{}).get("response",{}).get("responseFormat","json")
    if fmt=="file":
