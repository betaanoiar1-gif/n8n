...
class Executor:
 ...
 async def execute_node(self,name,item):
  ...
  if t=="n8n-nodes-base.httpRequest":
   resolved={"method":await self.ev(p.get("method","GET"),item),"url":await self.ev(p.get("url",""),item)}
   if p.get("sendHeaders"):
    resolved["headers"]={h["name"]:await self.ev(h.get("value",""),item) for h in p.get("headerParameters",{}).get("parameters",[]) if h.get("name")}
   if p.get("sendBody"):
    if p.get("jsonBody") is not None: resolved["body"]=await self.ev(p["jsonBody"],item)
    elif p.get("bodyParameters"): resolved["body"]=await self.ev(p["bodyParameters"],item)
  ...
  if t=="n8n-nodes-base.wait":
   seconds=await self.ev(p.get("amount",0),item)
   await asyncio.sleep(max(0,float(seconds or 0))); return [item],0
